...
function bind_proposal_vote_controls() {
  $('.proposal_vote_control').removeAttr('disabled').change(function(event) {
    target = $(this);
    $e = event; $t = target;
    ...
    value = target.attr('value');
    proposal_id = target.attr('x_proposal_id');
    vote_id = target.attr('x_vote_id')
    format = 'json';
    url = app.proposals_path + '/' + proposal_id + '/vote/' + vote_id + '.' + format;
	...
	target.attr('x_vote_id', response['id']);
	vote_avg = document.getElementById('proposal_vote_average_' + response['proposal_id']);
	if (vote_avg) {
	    vote_avg.firstChild.nodeValue = response['average'];
	}
      }	  
    });
